...
export class AppComponent {
  ...
  @Output() onHideCalendar = new EventEmitter<boolean>();
  ...
  hideButton(){
      ...
      this.onHideCalendar.emit(this.showCalendar);
  }
  ...
  if(this.events){
      ...
      for (; i<len; i++) {
        if (this.events[i].year == year && this.events[i].month == month && this.events[i].date == date) {
          ...
          return this.events[i].colors || ['#46b592'];
        }
      }
  }
  ...
  createMonth(year: number, month: number) {
      ...
      for (let i = 0; i < monthDays; i++) {
          this.dateArray.push({
              ...
              isToday: false,
              isSelect: false,
              hasEvent: (this.isInEvents(year, month, i+1)),
          })
      }
          ...
          for (let i = 0; i < nextMonthAdd; i++) {
              if (month === 11) {
                  this.dateArray.push({
                      ...
                      isToday: false,
                      isSelect: false,
                      hasEvent: (this.isInEvents(year, 0, i+1)) ,
                  })
              } else {
                  this.dateArray.push({
                      year: year,
                      ...
                      isToday: false,
                      isSelect: false,
                      hasEvent: (this.isInEvents(year, month+1, i+1)) ,
                  })
              }
